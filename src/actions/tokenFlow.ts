...
export const tokenFlowAction = {
  ...
  description:
    "Get net buy/sell flow for a Solana token over a rolling window (1h or 24h). Returns unique wallet/buyer/seller counts, buy/sell trade counts, buy/sell/net SOL, and trades-per-wallet. Default window is 1h. PRO/ULTRA only — BASIC receives HTTP 403.",
  examples: [
  ...
  handler: async (agent: unknown, input: { mint: string; window?: "1h" | "24h" }) => {
    try {
