...
export const streamSessionsAction = {
  ...
  description:
    "List your live WebSocket streaming sessions across ws-streaming and dex-stream. Returns { sessions, count }: each session has id, service (ws-streaming|dex-stream), tier, channels[], connected_at, remote_ip, and messages_sent. PRO/ULTRA only.",
  examples: [
...
export const streamSessionKillAction = {
  ...
  description:
    "Evict (kill) one of your live WebSocket streaming sessions by id — frees the connection slot. Returns { evicted: true, id }; 404 if no such session, 400 if id is not a positive integer. PRO/ULTRA only.",
  examples: [
  ...
  handler: async (agent: unknown, input: { id: string | number }) => {
    try {
