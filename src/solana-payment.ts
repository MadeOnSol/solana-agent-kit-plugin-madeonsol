...
// Keep emitted declarations independent of optional signing dependencies so
// API-key-only TypeScript consumers do not need to install @x402/core.
...
function fail(message: string): never { throw new Error(`Solana payment policy: ${message}`); }
...
function amount(value: unknown): bigint {
  ...
  if (parsed <= 0n || parsed > MAX_AMOUNT) return fail("amount exceeds uint64 range");
  return parsed;
}

function wallet(value: unknown): string {
  if (typeof value !== "string" || !/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(value)) return fail("invalid wallet address");
  ...
  if (decoded === 0n || bytes + (value.match(/^1*/)?.[0].length ?? 0) !== 32) return fail("invalid wallet address");
  return value;
}

function httpsUrl(value: unknown): URL {
  if (typeof value !== "string") return fail("invalid HTTPS URL");
  let url: URL;
  try { url = new URL(value); } catch { return fail("invalid HTTPS URL"); }
  if (url.protocol !== "https:" || url.username || url.password || url.hash) return fail("HTTPS URL without credentials or fragment required");
  return url;
}
...
export class SolanaPaymentBudget {
  ...
  constructor(policy: SolanaPaymentPolicy) {
    if (!policy) fail("paymentPolicy is required for keyless payments");
    ...
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 2_147_483_647) fail("invalid timeoutMs");
    if (policy.beforePayment !== undefined && typeof policy.beforePayment !== "function") fail("invalid beforePayment hook");
  ...
  get authorizedAmountAtomic(): string { return this.#authorized.toString(); }

  select(envelope: unknown, requestUrl: string): ApprovedChallenge {
    const challenge = envelope as { x402Version?: unknown; accepts?: unknown; resource?: { url?: unknown } | null } | null;
    if (!challenge || challenge.x402Version !== 2 || !Array.isArray(challenge.accepts) ||
        challenge.accepts.length < 1 || challenge.accepts.length > 32) fail("unsupported payment challenge");
    ...
    if (challenge.resource !== undefined) {
      ...
      if (resource.origin !== request.origin || resource.pathname !== request.pathname ||
          (resource.search && resource.search !== request.search)) fail("resource mismatch");
    }
    for (const offer of challenge.accepts) {
      if (!offer || offer.scheme !== "exact" || offer.network !== SOLANA_PAYMENT_NETWORK ||
          ...
          offer.extra?.feePayer !== this.policy.feePayer) continue;
      if (typeof offer.amount !== "string") continue;
      let cost: bigint;
      try { cost = amount(offer.amount); } catch { continue; }
      if (cost > this.#max || !Number.isSafeInteger(offer.maxTimeoutSeconds) || offer.maxTimeoutSeconds <= 0) continue;
      // Rebuild instead of forwarding arbitrary remote fields/extensions to a signer.
      const accepted: ApprovedOffer = Object.freeze({ scheme: "exact", network: SOLANA_PAYMENT_NETWORK,
  ...
  reserve(atomic: string): () => void {
    ...
    if (cost > this.#max || this.#authorized + cost > this.#total) fail("authorization budget exceeded");
    this.#authorized += cost;
    let released = false;
    return () => { if (!released) { this.#authorized -= cost; released = true; } };
  }
}

export function solanaPaymentPolicyFromConfig(get: (key: string) => string | undefined): SolanaPaymentPolicy {
  const required = (name: string): string => get(name) || fail(`${name} is required in keyless mode`);
  return {
...
async function challengeBody(response: Response): Promise<unknown> {
  ...
  if (header) {
    if (header.length > 65_536) fail("payment header too large");
  ...
  try {
    while (true) {
      ...
      if (size > 65_536) fail("payment challenge too large");
      chunks.push(value);
    }
  ...
  try { return JSON.parse(new TextDecoder().decode(bytes)); } catch { return fail("invalid payment challenge JSON"); }
}

/** One challenge and at most one paid request. No automatic payment replay. */
export async function createSolanaPaidFetch(
  privateKey: string, budget: SolanaPaymentBudget, baseUrl: string, transport: typeof fetch = fetch,
  ...
  if (signer.address === budget.policy.feePayer) fail("the agent wallet cannot pay facilitator gas");

  return async (input, init) => {
    const original = new Request(input, init);
    if (httpsUrl(original.url).origin !== origin) fail("request origin mismatch");
    if (original.headers.has("PAYMENT-SIGNATURE") || original.headers.has("X-PAYMENT")) fail("payment already attempted");
    const controller = new AbortController();
    const deadline = Date.now() + budget.policy.timeoutMs!;
    const check = () => {
      if (Date.now() >= deadline && !controller.signal.aborted) controller.abort(new Error("Solana payment timeout"));
    ...
    const bounded = async <T>(work: Promise<T>): Promise<T> => {
      const result = await Promise.race([work, cancelled]); check(); return result;
    };
    ...
    try {
      ...
      try {
        envelope = response.headers.has("PAYMENT-REQUIRED")
          ...
          : body;
      } catch { return fail("invalid payment challenge header"); }
      ...
      if (budget.policy.beforePayment) {
        ...
        if (approved !== true) fail("payment declined");
      }
      check();
      ...
      const guardedSigner: typeof signer = { ...signer, signTransactions: async (...args) => {
        check(); return signer.signTransactions(...args);
      } };
