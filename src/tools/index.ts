...
const authByAgent = new WeakMap<object, AuthEntry>();
...
const budgetByAgent = new WeakMap<object, SolanaPaymentBudget>();
function paymentBudget(agent: Agent): SolanaPaymentBudget {
  ...
  if (existing) {
    for (const key of ["payTo", "feePayer", "maxAmountAtomic", "maxTotalAmountAtomic", "rpcUrl"] as const) {
      if (policy[key] !== existing.policy[key]) throw new Error("Solana payment policy changed; use a new agent for a new allowance");
    }
    return existing;
  }
...
export function getAuthorizedPaymentAmount(agent: Agent): string {
...
function getAuth(agent: Agent): Promise<AuthContext> {
  ...
  const entry: AuthEntry = { apiKey, privateKey, context: createAuth(apiKey, privateKey, budget) };
  ...
  void entry.context.catch(() => {
    if (authByAgent.get(agent) === entry) authByAgent.delete(agent);
  });
  return entry.context;
}

async function createAuth(apiKey?: string, privateKey?: string, budget?: SolanaPaymentBudget): Promise<AuthContext> {
  if (apiKey) {
...
async function query(agent: Agent, path: string, params?: Record<string, string | number>) {
...
export async function kolFeed(
  ...
  return query(agent, "/api/x402/kol/feed", params as Record<string, string | number>);
}
...
export async function kolLeaderboard(agent: Agent, params: { period?: string; limit?: number } = {}) {
  return query(agent, "/api/x402/kol/leaderboard", params as Record<string, string | number>);
}
...
export async function deployerAlerts(
  ...
  return query(agent, "/api/x402/deployer-hunter/alerts", params as Record<string, string | number>);
}

export async function kolPairs(agent: Agent, params: { period?: string; min_shared?: number; limit?: number } = {}) {
  return query(agent, "/api/x402/kol/pairs", params as Record<string, string | number>);
}

export async function kolHotTokens(agent: Agent, params: { period?: string; min_kols?: number; limit?: number } = {}) {
  return query(agent, "/api/x402/kol/tokens/hot", params as Record<string, string | number>);
}

export async function kolTrendingTokens(agent: Agent, params: { period?: string; min_kols?: number; limit?: number } = {}) {
  return query(agent, "/api/x402/kol/tokens/trending", params as Record<string, string | number>);
}

export async function kolTokenEntryOrder(agent: Agent, params: { mint: string; limit?: number }) {
  const { mint, ...rest } = params;
  return query(agent, `/api/x402/kol/tokens/${encodeURIComponent(mint)}/entry-order`, rest as Record<string, string | number>);
}
...
export async function deployerAsOf(agent: Agent, params: { wallet: string; date?: string }) {
...
export async function deployerRewards(agent: Agent, params: { wallet: string }) {
...
export async function deployerLeaderboard(
  agent: Agent,
  params?: { tier?: string; sort?: string; limit?: number; offset?: number },
) {
 ...
 */
export async function deployerProfile(agent: Agent, params: { wallet: string }) {
...
export async function deployerTokens(
  agent: Agent,
  params: { wallet: string; limit?: number; offset?: number; only_bonded?: boolean },
) {
...
export async function deployerAlertStats(agent: Agent, params?: { period?: string }) {
  const qs = buildQs(params);
...
export async function deployerBestTokens(agent: Agent, params?: { period?: string; limit?: number }) {
  const qs = buildQs(params);
 ...
 */
export async function deployerRecentBonds(
  agent: Agent,
  params?: { limit?: number; since?: string; tier?: string; peak_mc_min?: number },
) {
  const qs = buildQs(params);
 ...
 */
export async function walletHoldings(agent: Agent, params: { address: string; limit?: number; min_value_usd?: number }) {
  const qs = new URLSearchParams();
 ...
 */
export async function tokenHolders(agent: Agent, params: { mint: string }) {
